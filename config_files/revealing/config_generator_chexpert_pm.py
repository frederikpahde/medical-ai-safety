...
config_dir = "config_files/revealing/chexpert"
...
with open("config_files/chosen_model_params.json", "r") as stream:
    chosen_model_params = json.load(stream)["chexpert"]

base_config = {
    'dataset_name': 'chexpert_pm',
    'img_size': 224,
    'method': 'AClarc',
    'wandb_api_key': 'YOUR_WANDB_KEY',
    'wandb_project_name': 'chexpert-pm-data-annotation'
}
...
def store_local(config, config_name):
    ...
    binary_target = config['binary_target']
    str_binary = "" if binary_target is None else f"_binaryTarget-{binary_target.replace(' ', '_')}"
