## Code inspired from https://github.com/lapalap/dora/blob/master/dora/dora.py
    ...
    def generate_signals(
        ...
        def make_custom_func(layer_number=0, channel_number=0, maximisation=True):
            ...
            def custom_func(layer_outputs):
                loss = layer_outputs[layer_number].squeeze(0)[channel_number].mean()
        ...
        with tqdm(total=len(task_list), disable=not (progress), desc="Generating s-AMS") as pbar:
            while counter < len(task_list):
                ...
                local_dreamer.set_custom_normalization(normalization_transform=fn_normalize)
...
class SignalDatasetRefData(torch.utils.data.Dataset):
    """Custom dataset class for loading the signals"""

    def __init__(self, fv, k, n, mode="relevance", layer_name=None, transform=None):
        """
        #TODO fill this
        """
        self.fv = fv
        ...
        assert mode in ["relevance", "activation"], f"mode must be either 'relevance' or 'activation', is: {mode}"
        max_path = fv.RelMax.PATH if mode == "relevance" else fv.ActMax.PATH
        max_sample_ids, _, _ = load_maximization(max_path, layer_name)

        self.metainfo = []

        for cid in range(k):
            for i in range(n):
                sample_id = max_sample_ids[i, cid]
                self.metainfo.append((sample_id, [cid, i, "+"]))

        print(f"Loaded metainfo for real reference samples ({len(self.metainfo)})")
    ...
    def __getitem__(self, idx):
        ...
        sample_id, details = self.metainfo[idx]

        sample = self.fv.dataset[sample_id][0]
...
class SignalDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir, k, n, only_maximization=True, transform=None):
        ...
        for x in sorted(glob.glob(f"{root_dir}/*.jpg")):
