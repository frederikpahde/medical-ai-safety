...
NORM_PARAMS_ISIC = ([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
...
def get_isic_dataset(data_paths,
    ...
    if normalize_data:
        fns_transform.append(T.Normalize(*NORM_PARAMS_ISIC))

    transform = T.Compose(fns_transform)

    return ISICDataset(data_paths, transform=transform, augmentation=isic_augmentation,
...
class ISICDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        self.normalize_fn = T.Normalize(*NORM_PARAMS_ISIC)
    ...
    def __getitem__(self, i):
        ...
        path = self.train_dirs_by_version[row.version] 
    ...
    def get_target(self, i):
        ...
        return self.classes.index(targets[targets == 1.0].index[0])

    def get_target_name(self, i):
        targets = self.metadata.loc[i]
        target = targets[targets == 1.0].index[0]
        return target
    
    def get_class_id_by_name(self, class_name):
        columns = self.metadata.columns.to_list()
        class_id = columns.index(class_name) - 1
        return class_id
