...
def get_resnet18(ckpt_path=None, pretrained=True, n_class: int = None, device="cuda") -> torch.nn.Module:
    return get_resnet(resnet18, ckpt_path, pretrained, n_class, device)


def get_resnet34(ckpt_path=None, pretrained=True, n_class=None, device="cuda") -> torch.nn.Module:
    return get_resnet(resnet34, ckpt_path, pretrained, n_class, device)


def get_resnet50(ckpt_path=None, pretrained=True, n_class: int = None, device="cuda") -> torch.nn.Module:
    return get_resnet(resnet50, ckpt_path, pretrained, n_class, device)

def get_resnet101(ckpt_path=None, pretrained=True, n_class: int = None, device="cuda") -> torch.nn.Module:
    return get_resnet(resnet101, ckpt_path, pretrained, n_class, device)


def get_resnet(model_fn, ckpt_path=None, pretrained=True, n_class: int = None, device="cuda") -> torch.nn.Module:
    if pretrained:
        weights = "DEFAULT"
    else:
        weights = None
